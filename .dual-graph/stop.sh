...
if input_tokens > 0 or cache_create > 0 or cache_read > 0 or output_tokens > 0:
    print(json.dumps({
        ...
        "description": "auto",
        "project": "/home/jacques/Documents/DEV/PERSO/tictacdoh",
    }))
PYEOF
)
  if [[ -n "$USAGE" ]]; then
    # POST to MCP graph server (always running, reliable)
    MCP_PORT=$(cat "/home/jacques/Documents/DEV/PERSO/tictacdoh/.dual-graph/run/claude/mcp_port" 2>/dev/null || echo "8081")
